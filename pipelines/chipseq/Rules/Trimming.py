...
rule trim_galore_pe:
    ...
    shell: 
        "{params.trim_galore} -o {PATH_TRIMMED}/{wildcards.sample} --paired {input.infile[0]} {input.infile[1]} >> {log} 2>&1 ; mv {params.tmp1} {output.r1} && mv {params.tmp2} {output.r2}"
...
rule trim_galore_se:
    ...
    log: 
        os.path.join(PATH_LOG, 'trim_galore_{sample}.log')
    shell: 
        "{params.trim_galore} -o {PATH_TRIMMED}/{wildcards.sample} {input.infile[0]} >> {log} 2>&1 ; mv {params.tmp} {output.read}"
