...
#for all rmd files except index.Rmd, create a navbar item (for each target_name in sample sheet)
navbar_yml$left <- c(navbar_yml$left, lapply(grep("^index.Rmd$", basename(rmd_files), invert = T, value = T), 
       function(f) {
