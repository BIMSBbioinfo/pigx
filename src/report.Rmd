...
deletionCutsiteOverlaps <- deletionCutsiteOverlaps[ReadSupport > 5]
...
plots <- lapply(unique(deletionCutsiteOverlaps$sample), function(s) {
    ...
    if(sgRNAs[1] != 'none') {
      plots <- lapply(sgRNAs, function(sgRNA) {
      tmp <- dt[get(sgRNA) == 1]
      if(nrow(tmp) > 0) {
        tmp <- tmp[order(end-start)]
        tmp$linePos <- 1:nrow(tmp)/100
        p1 <- ggplot(tmp, aes(x = linePos, ymin = start, ymax = end)) + 
          geom_linerange(size = 0.5) + 
          ylim(c(min(tmp$start)-10, max(tmp$end)+10)) + 
          theme(axis.text.y = element_blank(), axis.ticks.y = element_blank()) +
          labs(x = '', y = 'base position\n(cut site displayed as grey bar)', 
               title = paste0('unique deletions n=',nrow(tmp))) + 
          geom_point(data = tmp, aes(x = linePos, y = start), size = 0.05, color = 'red') + 
          geom_point(data = tmp, aes(x = linePos, y = end), size = 0.05, color = 'red') + 
          geom_hline(yintercept = cutSites[cutSites$sgRNA == sgRNA,]$cutSite, 
                     color = 'black', size = 5, alpha = 0.2) + 
          coord_flip()
        p2 <- ggplot(tmp, aes( x = get(sgRNA), y = end - start + 1)) + 
          geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) + 
          geom_jitter(height = 0, alpha = 0.5, 
                      aes(size = as.factor(round(log10(tmp$ReadSupport))), 
                          color = as.factor(round(log10(tmp$ReadSupport))))) + 
          labs(x = sgRNA, y = 'Deletion Length', size = 'log10(Read Support)', color = 'log10(Read Support)') + 
