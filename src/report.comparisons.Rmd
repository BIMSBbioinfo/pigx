...
printBedFile <- function(outFile, trackText, df) {
  writeLines(text = trackText,
             con = outFile)
...
parallel::clusterExport(cl = cl, varlist = c('comp', 'comparePerBaseScores', 'coverageStats'))

results <- as.data.frame(do.call(rbind,  pbapply::pblapply(cl = cl, X = comp$comparison, 
                                                           FUN = function(x) {
  require(data.table)
