...
params:
  ...
  sceRdsFile: /data/local/buyar/pigx/scrna/integrate_report/test.RDS
  workdir: .
---
...
plotPCAcontributorsHeatmap = function(workdir, sce, cellCount = 500, pc, n = 20){
  ...
  exprMatrix <- assay(sce, 'cpm')[which(rowData(sce)$Genes %in% topGenes),select]
  ...
  p <- pheatmap::pheatmap(mat = exprMatrix, scale = 'column', 
                     show_rownames = TRUE, 
