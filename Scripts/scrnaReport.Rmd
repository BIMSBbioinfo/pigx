...
reducedDimPlot <- function(df, dim1, dim2, title = NULL, color_by = NULL) {
  p <- ggplot2::ggplot(df, aes_string(x = dim1, y = dim2)) +
    geom_point(aes_string(color = color_by), size = 0.5) +
    labs(title = title)
  return(p)
}
```
...
pcaPlotList <- lapply(covariates, function(cov) {
  pL<- lapply(paste0('PC', c(2:8)), function(pc) {
  myTitle <- paste('PC1 vs', pc)
  reducedDimPlot(df = pcaData, dim1 = 'PC1', dim2 = pc, title = myTitle, color_by = cov)
  })
  names(pL) <- paste0('PC1 vs PC',c(2:8))
  return(pL)
  }
)
names(pcaPlotList) <- covariates
```
