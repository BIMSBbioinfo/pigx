...
message("covariates:",params$covariates,' vs ',covariates)
...
mapIdsToNames <- function(ids, gtfData) {
  ...
  dt <- unique(data.table('transcript_id' = transcripts$transcript_id, 
                   'gene_id' = transcripts$gene_id, 
                   'gene_name' = transcripts$gene_name, stringsAsFactors = FALSE))
  m <- apply(head(dt[,1:2], 1000), 2, function(x) sum(x %in% ids))
  #then map the ids to gene names
  if(m['transcript_id'] > m['gene_id']){
    return(dt[match(ids, dt$transcript_id),]$gene_name)
  } else {
    return(dt[match(ids, dt$gene_id),]$gene_name)
  }
}

if(length(covariates) > 0){
  message(paste("something wrong with design length:", length(covariates),"=>",covariates))
  designFormula <- paste("~", paste(covariates, collapse = ' + '), "+ AnalysisGroup")
} else {
  designFormula <- "~ AnalysisGroup"
}

message("coldata columns:",colnames(colData))
