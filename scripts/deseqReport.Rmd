...
#switch to control whether GO analysis module should be run
runGO <- ifelse(organism %in% c('human', 'mouse', 'worm', 'fly'), TRUE, FALSE)
...
```{r results='asis'}
if(runGO == FALSE) {cat("Warning:Skipping GO analysis as organism:",organism,"is not supported\n")}
```
...
runGOanalysis <- function(targetedGenes, backgroundGenes, species, writeToFile = NULL) {
...
genesUp <- rownames(DE[!is.na(DE$padj) & DE$padj < 0.1 & DE$log2FoldChange > 1,])
genesDown <- rownames(DE[!is.na(DE$padj) & DE$padj < 0.1 & DE$log2FoldChange < -1,])
allGenes <- rownames(DE)
```
...
```{r, eval = runGO}
if(length(genesUp) > 5){
  goUp <- runGOanalysis(targetedGenes = genesUp, backgroundGenes = allGenes, species = organism)
  DT::datatable(goUp, 
          extensions = c('Buttons', 'FixedColumns'), 
          options = list(fixedColumns = TRUE, 
                         scrollX = TRUE,
  ...
  cat("Couldn't find at least 5 up-regulated genes for GO analysis\n")
}
```
...
```{r, eval = runGO}
if(length(genesDown) > 5) {
  goDown <- runGOanalysis(targetedGenes = genesDown, backgroundGenes = allGenes, species = organism)
  DT::datatable(goDown, 
          extensions = c('Buttons', 'FixedColumns'), 
          options = list(fixedColumns = TRUE, 
                         scrollX = TRUE,
  ...
  cat("Couldn't find at least 5 down-regulated genes for GO analysis\n")
}
```
