...
runGO <- ifelse(species %in% c('human', 'mouse', 'worm', 'fly'), TRUE, FALSE)
...
```{r results='asis'}
if(runGO == FALSE) {cat("Warning:Skipping GO analysis as species:",species,"is not supported\n")}
```
...
```{r, eval = runGO}
if(length(genesUp) > 5){
  goUp <- runGOanalysis(targetedGenes = genesUp, backgroundGenes = allGenes, species = species)
  DT::datatable(goUp, 
          extensions = c('Buttons', 'FixedColumns'), 
          options = list(fixedColumns = TRUE, 
                         scrollX = TRUE,
...
if(length(genesDown) > 5) {
  goDown <- runGOanalysis(targetedGenes = genesDown, backgroundGenes = allGenes, species = species)
  DT::datatable(goDown, 
          extensions = c('Buttons', 'FixedColumns'), 
          options = list(fixedColumns = TRUE, 
                         scrollX = TRUE,
