...
This plot displays the numbers of mapped reads (in millions) for total (green bars) and uniquely-mapped UMI (orange bars) per each sample.
...
This plot displays the numbers of mapped reads (in millions) categorized by annotation (as indicated by the legend) for each sample.
...
The following plots show the scores obtained through PCA for the pairs of components indicated in the axes. These plots can be used to assess the data structure as well as to detect clusters, trends and outliers. The sample points are colored by the feature indicated in the corresponding title.
...
The following plots show the scores obtained through t-SNE analysis for the pairs of components indicated in the axes. These plots can be used to assess the data structure as well as to detect clusters, trends and outliers. The sample points are colored by the feature indicated in the corresponding title.
...
The following plots display the scores of top genes contributing to the given principle components. x-axis indicates the score for the given principle component while y-axis indicates the genes contributing significantly to the component.
...
The following plots display heatmaps of the expression values of top contributing genes to the given principle component. Columns are cells and rows are the top genes. _CellCyclePhase_ indicates cell cycle phases classified based on gene expression data. _log10nGene_ indicates log10-transformed numbers of genes detected per cell. _replicate_ indicates replicates.
