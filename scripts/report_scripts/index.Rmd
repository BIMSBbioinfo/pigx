...
if ( !(length(coverages.df) == 0)){
    good_samples.df <- coverages.df %>% filter( coverage >= 90) # TODO stringecy should be given by user, maybe even by visualization
    bad_samples.df <- coverages.df %>% filter( coverage < 90)
  # 90 was chosen because samples with 90% cov often still cover all amplicons
    
    # table displaying bad samples which are not further inlcuded 
    str <- "\n Samples without sufficient genome coverage. They aren't included in the visualization or the linear regression" # TODO refine this text, put into seperate chunk
