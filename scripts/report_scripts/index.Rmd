...
approved_var_plot_pooled <- pool_by_weighted_mean(approved_var_plot, weights)
...
if (nrow(approved_mut_plot) > 0){
  ...
  # filter good samples for only mutations with sig pvalues for plotting
    ...
    approved_mut_plot_pooled <- pool_by_mean(approved_mut_plot, na_handling = TRUE)
    mutations_sig <- mutations_sig %>% slice_head(n = 20)
...
count_frame <- count_frame %>% slice_head(n=20)
...
mutations_sig_unfiltered <- mutations_sig_unfiltered %>% slice_head(n=20)
