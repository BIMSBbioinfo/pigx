...
# prepare the frames for downloadable output
count_frame <- count_frame %>% paste0(collapse="\n") %>% base64_urlencode()
mutations_sig_unfiltered <- mutations_sig_unfiltered %>% paste0(collapse="\n") %>% base64_urlencode()
``` 
[Download Mutation_counts.csv](`r sprintf('data:text/csv;base64,%s', count_frame)`)
[Download linear_regression_results.csv](`r sprintf('data:text/csv;base64,%s', mutations_sig_unfiltered)`)
