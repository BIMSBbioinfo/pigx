...
#test
sample_sheet <- data.table::fread("../../tests/sample_sheet.csv")
...
df <- as.data.frame(dplyr::select(sample_sheet,location_name, date, time, name))
...
df <- reshape2::melt(df, id.vars= c("location_name","date","time","name"), value.name = "link")
df <- df[,-5]
```
