...
for (i in all_mutations){
  if (i %in% complete.df$AA_mut ){ # split gene name to match with AA mut
    ...
    if ( i %in% colnames(output_mutation_frame)) {
        output_mutation_frame[sample_row,i] <- complete.df$freq[ which(complete.df$AA_mut == i)]
}}}
...
# convert numeric values to character
output_mutation_frame <- as.data.frame( lapply( output_mutation_plot, as.character ), check.names = FALSE)
