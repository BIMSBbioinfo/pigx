...
SignatureTable <-  data.frame("SNV" = match.df$gene_mut.x, 
                  "gene" = match.df$genes,
...
NoSignatureTable <-  data.frame("SNV" = nomatch.df$gene_mut.x, 
...
condensed_variants_names <-
    unlist(lapply(variants, function (x) { if (str_detect(x, ",{4,}")) { "others" } else { x }}))
