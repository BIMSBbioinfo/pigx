...
complete.df <- complete.df %>% # ! hack? (to discard genes)
  group_by(across(c(-freq,-gene_mut,-genes, -gene_pos, -gene_mut_loc.2, -cov, AA_mut))) %>%
  summarise(freq = sum(as.numeric(freq)),
            gene_mut = paste(gene_mut, collapse = ","), genes = paste(genes, collapse = ","),
            gene_pos = paste(gene_pos, collapse = ","), 
            gene_mut_loc.2 = paste(gene_mut_loc.2, collapse = ","), 
