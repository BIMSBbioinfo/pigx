...
all_mutations <- complete.df$AA_mut[!is.na(complete.df$AA_mut)]
...
for (i in all_mutations){
  if (i %in% complete.df$AA_mut ){ # split gene name to match with AA mut
    # check if variant already has a column
    if ( i %in% colnames(output_mutation_plot)) {
        output_mutation_plot[sample_row,i] <- complete.df$freq[ which(complete.df$AA_mut == i)]
}}}
