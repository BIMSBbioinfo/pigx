...
# fixme Not sure if there could be a more elegant way to do this
# one aa mutation can have different codon mutations reported with different freqs- those have to be summed up
# This introduces a weird bug I think --> see line 1872 of documentation, since we carry the gene names now with us
# mabye we don't need this anymore --> remove and check if is is maybe the course for the ones sample weird behavior
...
if ( length(msig_dedupe_transposed) >= 1){ 
    ...
    dropped_variants <- unique(dropped_variants)[!is.na(dropped_variants)]
}
...
# add dropped variants again with value 0 in case all of the other variants add up to 1
...
# in case "others" == 0, both variants can be split up again and being given the value 0 # ? I think so?
...
all_mutations <- paste(complete.df$genes, complete.df$AA_mut, sep = ":")
...
for (i in all_mutations){
  if (str_split(i,':')[[1]][2] %in% complete.df$AA_mut ){ # split gene name to match with AA mut
    # check if variant already has a column
    if ( i %in% colnames(output_mutation_plot)) {
        output_mutation_plot[sample_row,i] <- complete.df$freq[ which( complete.df$AA_mut == str_split(i,':')[[1]][2])]
}}}
colnames( output_mutation_plot ) <- as.character( colnames( output_mutation_plot ))
output_mutation_plot <- output_mutation_plot %>% dplyr::select( -contains("NA" , ignore.case = FALSE))
# 2. check if file exists already
if (file.exists (output_mutations)) {
  previous_df <- read.table (output_mutations, 
                             sep = "\t", header = TRUE, colClasses = "character", check.names = FALSE)
  # convert numeric values to character
  output_mutation_plot <- as.data.frame( lapply( output_mutation_plot, as.character ), check.names = FALSE)
  # merge with adding cols and rows
  output_mutation_plot <- full_join(previous_df, output_mutation_plot, 
                                    by = intersect(colnames(previous_df), colnames(output_mutation_plot)), copy = TRUE)
}

# 3. write to output file
   write.table(output_mutation_plot, output_mutations, sep = "\t",
