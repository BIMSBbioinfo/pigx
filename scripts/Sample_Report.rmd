...
# infile = '/data/local/Projects/pigx/pigx_chipseq/Data/Analyzed/Analysis/Summarized_Data_For_Report.RDS'
# lstats = list()
...
# ---------------------------------------------------------------------------- #
# Figure sizes
sample_correlation_width  = number_of_samples + 5
sample_correlation_height = number_of_samples
...
# Sample Correlation
```{r Sample_Correlation, include=TRUE, fig.align='center', message=FALSE, warning=FALSE, fig.width=sample_correlation_width, sample_correlation_height}
...
# Cross Correlation  {.tabset}
```{r Cross_Correlation, include=TRUE, fig.align='center'}
for(sample in unique(lstats$ChIPQC$shift_correlation$sample_name)){
...
genomic_coordinates = lapply(genomic_locations, function(x){
...
for(i in seq(genomic_locations)){
  ...
  g = tab %>%
    ...
    ylab('Signal Value') +
    scale_y_continuous(breaks=coordinates)
  
    if(coordinates[2] == 0)
        g = g + geom_vline(xintercept = 0, linetype=2, size=1)
  print(g)
  cat('\n')
}
```
